...
import { BUSINESS_DOMAINS, ENTITIES, APPLICATIONS, TRACKS } from './masterData';
import { RESOLVER_GROUPS, RESOLVER_TIERS, AGEING_BUCKETS } from './config';
...
// ═══════════════════════════════════════════════════
// AUDITS, FINDINGS, CTAs — DEMO
// ═══════════════════════════════════════════════════
function generateAudits() {
  const audits = [];
  const auditTypes = ['Internal', 'External', 'Regulatory', 'Compliance'];
  const auditStatuses = ['Planned', 'In Progress', 'Completed', 'Cancelled'];
  const complianceStatuses = ['Compliant', 'Partially Compliant', 'Non-Compliant', 'Under Review'];

  for (let i = 1; i <= 12; i++) {
    const domain = BUSINESS_DOMAINS[i % BUSINESS_DOMAINS.length];
    const startDate = new Date(2026, 3 + (i % 6), 1 + (i % 28));

    audits.push({
      id: `AUD-${String(i).padStart(4, '0')}`,
      auditName: `${domain.label} Process Compliance Audit Q${Math.ceil((i % 4 + 1))} 2026`,
      type: auditTypes[i % auditTypes.length],
      businessDomain: domain.key,
      owner: RESOURCES[i % RESOURCES.length].name,
      plannedStart: startDate.toISOString().split('T')[0],
      plannedEnd: new Date(startDate.getTime() + 14 * 24 * 3600 * 1000).toISOString().split('T')[0],
...
function generateFindings() {
  const findings = [];
  const impactCategories = ['Critical', 'Major', 'Minor', 'Observation'];
  const findingStatuses = ['Open', 'In Progress', 'Remediated', 'Closed', 'Overdue'];

  for (let i = 1; i <= 24; i++) {
    const domain = BUSINESS_DOMAINS[i % BUSINESS_DOMAINS.length];

    findings.push({
      id: `FND-${String(i).padStart(4, '0')}`,
      shortDescription: getFindingDescription(i),
      impactCategory: impactCategories[i % impactCategories.length],
      businessDomain: domain.key,
      processGroup: RESOURCES[i % RESOURCES.length].processGroup,
      assignedTo: RESOURCES[(i + 2) % RESOURCES.length].name,
      targetDate: new Date(2026, 6 + (i % 4), 1 + (i % 28)).toISOString().split('T')[0],
      complianceStatus: findingStatuses[i % findingStatuses.length],
      auditId: `AUD-${String(((i - 1) % 12) + 1).padStart(4, '0')}`,
      ctaId: `CTA-${String(i).padStart(4, '0')}`,
      classification: 'DEMO',
    });
  }
  return findings;
...
function getFindingDescription(i) {
  const descriptions = [
    'Segregation of duties violation in approval workflow',
    'Missing change documentation for transport requests',
    'Incomplete disaster recovery test evidence',
    'Unauthorized access pattern detected in audit log',
    'Non-compliant data retention configuration',
    'Missing vendor due diligence documentation',
    'Incomplete security patch deployment records',
    'Process deviation from documented SOP',
  ];
  return descriptions[i % descriptions.length];
}
